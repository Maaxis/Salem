...
class Admin(commands.Cog):
	...
	async def user_is_admin(self, interaction):
		...
		await utils.log(
			f"{interaction.channel.mention}: {interaction.user.mention} tried to use an admin command but was not an admin.",
			self.bot, warning=0)
		return False
	...
	async def say(self, interaction: discord.Interaction, channel: discord.TextChannel, msg: str):
		...
		await utils.log(
			f"{interaction.channel.mention}: Message sent to {channel.mention} by {interaction.user.mention}.",
			self.bot, warning=0)
