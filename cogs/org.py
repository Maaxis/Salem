...
class Game(commands.Cog):
    ...
    async def current_vl_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        if not await user_is_admin(interaction):
            await interaction.response.send_message("You need to be an admin and in the SH10 server to use this command.")
            return
        try:
    ...
    async def handle_member_join(self, member): # ping hosts on server member join
        if member.guild.id == get_config('server_id'):
            try:
                ...
                await log(f"An error occurred in handle_member_join: {e}", self.bot, warning=2, dump=traceback.format_exc())
    ...
    async def handle_member_remove(self, member): # ping hosts on server member leave
        if member.guild.id == get_config('server_id'):
            try:
                ...
                await log(f"An error occurred in handle_member_remove: {e}", self.bot, warning=2, dump=traceback.format_exc())
    ...
    async def handle_thread_update(self, before, after): # for private threads, keep locked / uninvitable by non thread owner
    ...
    async def handle_thread_member_join(self, member): # for alliance threads, don't allow new users to join after 10min
        try:
            ...
            await log(f"Something went wrong in handle_thread_member_join: {e}", self.bot, warning=2, dump=traceback.format_exc())
    ...
    async def handle_member_update(self, before, after): # for adding new viewers to alliance threads
        if after.guild.id == get_config('server_id'):
            if not self.bot.paused:
                if get_config('viewer_role_id') in [role.id for role in after.roles] and get_config('viewer_role_id') not in [role.id for role in
                                                                                            before.roles]:
                    try:
                        await log(
