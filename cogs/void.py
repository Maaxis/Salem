...
class Void(commands.Cog):
	...
	async def icon(self, interaction: discord.Interaction, label: str = None, emoji: str = None, image_url: str = None,
		...
		if image_file and not image_file.content_type.startswith("image/"):
			await interaction.followup.send("Gimme a REAL IMAGE, silly hooman! (PNG or JPG, or the spell FIZZLES!)",
			                                ephemeral=True)
			await log(
				f"/icon: User tried to upload an image file but the file doesn't seem to be valid (filename: {image_file.filename})",
				self.bot, 1)
			return
		...
		if emoji:
			...
			try:
				...
				async with aiohttp.ClientSession(headers=headers, connector=connector) as session:
					async with session.get(image_url) as resp:
						...
						if resp.status != 200:
							...
							await log(
								f"/icon: User provided image URL but something went wrong trying to access the URL (URL <{image_url}>, status code {resp.status})",
								self.bot, 1)
							return
				...
				await interaction.followup.send(f"UH, is this URL possessed? Check it and try again! {image_url}",
				                                ephemeral=True)
				await log(f"/icon: Exception while downloading image: {e}", self.bot, warning=2,
				          dump=traceback.print_exc(), mention=True)
				return
		...
		try:
			...
			import traceback  # TODO: why doesn't this work if traceback is outside? lmao
			traceback.print_exc()
			await log(f"/icon: Something went wrong while creating the icon role: {e}", self.bot, warning=2,
			          dump=traceback.print_exc(), mention=True)
			return
	...
	async def color(self, interaction: discord.Interaction, color: str = None, second_color: str = None):
		try:
			...
			if await not_void(interaction):
				await log(
					f"/color: returned because the interaction was not in the VOID server (server id: {interaction.guild.id})",
					self.bot, 0)
				return
			...
			if not color:
				...
				await interaction.followup.send("Your color? VANQUISHED like the smallest tribe in a swap.",
				                                ephemeral=True)
				return
			color = color.replace("#", "")
			if second_color:
				...
				try:
					...
					if second_color:  # TODO: update to use non overwritten version
						r = await interaction.guild.create_role(
							name="#" + color.upper() + " to #" + second_color.upper(),
							color=discord_color,
